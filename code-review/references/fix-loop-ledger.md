...
- `closure evidence`: implementation path, regression/negative test, and
  validation command/result. State why a test is not possible when applicable.
...
| Source rewrites             | parser span, duplicate text, prefix/suffix, fence, inline code, reference form, generated file       |
...
- Every repeated Action theme has a regression test, or a written reason one is
  not possible.
