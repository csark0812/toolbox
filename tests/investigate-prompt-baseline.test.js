...
describe('investigate prompt baseline', () => {
  ...
  it('prompt and transfer arms share the same hygiene seeds as outcomes', () => {
    for (const compareId of outcome.scenarios.map((s) => s.compareId)) {
      ...
      expect(promptRow.seedPatch).toBe(outcomeRow.seedPatch)
      expect(transferRow.seedPatch).toBe(outcomeRow.seedPatch)
    }
  })
})
