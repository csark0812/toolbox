...
Read [references/research-basis.md](references/research-basis.md) when you calibrate blindness or exit claims. Do not load by habit.
...
- If there is no bounded slice, stop. Ask the user to name scope. Do not substitute full-repo or full-plan work.
...
3. **Exit** only when [exit-gate.md](references/exit-gate.md) is satisfied — `Closure: ready`.
...
3. **Coordinator synthesis** — user-facing output per [output.md](references/output.md) in pragmatic STE. Thrash check against [thrash-ledger.md](references/thrash-ledger.md) (inventory **and** novelty).
4. **Disposition / fix** — `acted` implements. `deferred-to-user` soft-stops for ask. `declined` with why. Update ledger and streak.
5. **Pass progress** — after every blind Task, pragmatic-STE Found / Worth acting / Delta / Still blocks exit ([output.md](references/output.md#pass-progress-required-after-every-blind-task)). Never forward this to the blind reviewer.
