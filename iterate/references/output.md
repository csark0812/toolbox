...
- [what still fails exit-gate, in plain language] — or soft-stopped (diminishing-returns / deferred-to-user) — or none if streak reaches M / gate otherwise green
```
...
Portable suites may assert presence of: `Pass: blind`, `Slice:`, `Clean streak`, `Cohesion: attested-local`, `Thrash: inventory-required`, `Thrash: diminishing-returns`, `### Round`, `**Found:**`, `Worth acting`, `deferred-to-user`. Suites do **not** prove Task spawn or blindness isolation.
