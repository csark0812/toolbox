...
Independent web topics in parallel. Uses via subagents for spawn — non-negotiables, member prompts, member schema (load the subagents skill).
...
Prefer Auto for all topics (`inherit-auto` or `model=auto` if enum supports it). Explicit model slugs require slice-fit + Cursor cost justification per via subagents (model-routing). Do not use `*-fast` in parallel. Diversify via distinct prompts/stances, not price.
...
4. High-stakes contradiction → sequential tiebreaker per via subagents (model-routing) or ask user.
5. Output → via subagents (output-format).
