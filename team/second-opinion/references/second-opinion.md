...
| **B — Completeness verify** | "Verify my plan", "did I miss anything", readiness on `.plan.md` / `docs/prds/` / issues; **axis checklist**, fixed report shape, does not rewrite | [planning/verify.md](../../references/planning/verify.md) |

**Both:** If you want outsider critique **and** the axis checklist, run **A then B**, or **B then A** if premises are already settled.

**Not in scope for either stance:** security/compliance review (use **security** agent or [`docs/security/audit.md`](../../../../docs/security/audit.md)), reproducible broken behavior (**investigate** + **testing** + **debug** when layer or session logs unclear). For broad proactive codebase sweeps, use [parallel-explore.md](../../references/planning/parallel-explore.md).
...
Synced with [dialogue-contract.md](../../references/dialogue-contract.md) § Structural checks — plan-review depth here; do not duplicate bullets; update dialogue-contract if patterns change.
...
- **Local change** vs **staged or ground-up** — cite [dialogue-contract.md](../../references/dialogue-contract.md) patterns when naming the call.

### Product fit (brief)

Synced with [dialogue-contract.md](../../references/dialogue-contract.md) § Product checks — plan-review depth here; do not duplicate bullets.
