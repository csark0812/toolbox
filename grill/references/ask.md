...
[1–3 plain sentences: what we’re in the middle of — written for a skimming human, not an agent.
...
1. **Self-contained** — other grill chats may be open; everything needed to answer lives in this ask block (Context + Questions).
2. **Human-first Context** — Context is for the user. After reading it (before the questions), they should know what conversation they’re in. Ban skill jargon here (`thrash`, `Closure`, `soft stop`, `falsifier`, theme ids, etc.) — say the plain idea instead. Prefer everyday words over labels the skill invented.
3. **Lean Context** — keep the `## Context` heading:
   - Lead with 1–3 plain sentences (required; never a bare heading)
   - Add **Already agreed:** only for locks that change how they’d pick; omit the whole block when empty
   - Do not restate what the `###` stem(s) already ask
   - Do not use Where / Deciding / Settled keys — those labels are retired
4. **Plain language in questions** — no skill jargon in stems either.
...
9. **`Other:`** only when the option set may be incomplete (not on every question).
...
We’re tightening how grill asks look in chat. You often hop between several grill threads, so each ask has to make sense on its own.

**Already agreed:**
- Questions use lettered picks with a recommended option.
- Each ask has to stand alone because you hop between grill threads.
...
> **Recommended: A** — the next turn can restate what’s already agreed; a footer often repeats noise
```
...
### 1. How should clients page through notes?
...
> **Recommended: A** — stays cheap when the list grows; clients don’t skip/duplicate as easily
> Why it matters: This choice locks the response shape and how painful “page 50” gets later.

---

### 2. What’s the default page size?
...
> Why it matters: Default size hits every caller who doesn’t opt in.
```
