---
name: crystallize
description: Shape fuzzy intent through Socratic dialogue — crystallized statement, not solutions or a plan yet. Process skill; dialogue-only. Not design pressure-test (grill), written plan review (second-opinion), or code hunch (probe).
---
...
- User has **fuzzy intent** — half-formed idea, not a concrete hunch or written plan.
- Specific code-path doubt or repro bug → stop; use **probe** (Evidence or Fix per Authority B).
- Written plan on disk to critique → stop; use **second-opinion**.

## Non-negotiables

1. **Crystallization not resolution** — no architecture unless user asks.
2. **One cluster per turn** — no unrelated question dumps.
3. **Gate before final block** — assumption or alternate frame surfaced first.
...
Per [output.md](references/output.md) — crystallized statement, open threads, suggested next atoms.
