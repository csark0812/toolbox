...
**Hard rule (same wave):** Members do not communicate. Never use identical model + identical prompt.

**Staged exception:** Wave 2 can receive **coordinator-composed briefs** of wave-1 findings only — not live inter-member chat. That is the intentional back-and-forth.
...
[Why this claim must ship-block, be dropped, or be narrowed — or "n/a"]
...
| Stance     | Mandate                                                              |
| ---------- | -------------------------------------------------------------------- |
| `attacker` | Strongest reasons to reject the claim or to establish the risk       |
| `refuter`  | Kill weak or false findings. State mechanisms that prevent the risk. |

Code-review: keep lens selection. Give every member a kill mandate for their lens. Budget ≥ 2 reserves one `refuter` slot (prefer dropping lowest-scored **optional** lens, never a required agent). Quick (1) = single lens with attacker mandate only.
...
- Single-member orchestrator spawns (for example, **iterate** blind pass) → [carve-out](../SKILL.md#entry-skill-carve-out), not this section.
