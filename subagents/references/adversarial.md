...
Med and deep recipes → [second-opinion-dispatch.md](second-opinion-dispatch.md). Synthesis shape → [second-opinion plan-review.md](../../second-opinion/references/plan-review.md).

- **Default shape** for **second-opinion** on a written artifact (path or paste) — invent lenses, then **light** (coordinator-only), **med** (1+1), or **deep** (multi-lens + defend) per plan-review routing; spawn only selected members.
...
Optional pre-wave gather → [second-opinion-evidence-dispatch.md](second-opinion-evidence-dispatch.md) (large **deep** only).
