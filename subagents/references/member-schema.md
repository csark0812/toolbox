...
**When `N = 1`:** use this schema only if the entry skill does not specify output (e.g. generic explore gather). When an entry skill defines output (iterate blind pass, second-opinion debate member fields, verdict.md synthesis), **entry skill output wins**.
