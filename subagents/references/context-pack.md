...
**Overlap rule:** Same `Slice:` → **tdd** (red-green cycles), **iterate** (blind cohesion passes), **code-review** (evidence filing) can all apply without any skill naming another. Different primitives → gates apply independently (for example, **second-opinion** needs an **Artifact**. **tdd** needs a **Seam**).
...
4. **Context asymmetry** — members do not get the full user thread, coordinator synthesis, or other members' raw transcripts. Exception: the entry skill explicitly allows structured briefs (for example, second-opinion wave 2).
...
- Return only your mandate. The coordinator synthesizes.
