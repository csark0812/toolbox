...
export function classifyD1NullArmResult(input) {
	...
	if (categories.some((c) => c === 'agent_runtime' || c === 'worktree_leak' || c === 'judge_infra')) {
		return 'infra'
	}
	...
	const forageMatchers = matchers.some(
		(m) => m === 'toHaveNotReadPath' || m === 'mustNotReadPath',
	)
	const forageMsg =
	...
	const forageTranscript =
		/null-arm-hygiene\.patch|diagnose\/SKILL\.md|Entry gate — no loop/i.test(transcript)
...
export async function summarizeD1NoneForensics(noneRows) {
	...
	const cleanNoneFail = inventFails > 0 && inventFails >= forageFails && inventFails >= Math.ceil(failCount / 2)
...
export function decideDiagnoseD1Disposition(input) {
	...
	const fullBeatsPrompt =
		input.d1FullBeatsPrompt > input.d1PromptBeatsFull && !promptMatchesFull

	if (forensics.forageConfound || (fullBeatsNone && forensics.cleanNoneFail === false && (forensics.inventFails ?? 0) === 0)) {
		return {
	...
	if (!fullBeatsNone || input.d1NoneWins >= input.d1FullWins || input.d1PromptBeatsFull > input.d1FullBeatsPrompt) {
		return {
	...
	return {
		...
		claimReady: false,
		rationale: 'Signal incomplete (ties, infra, or mixed forensics) — invest more before Keep/Remove.',
	}
}
