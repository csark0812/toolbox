...
- **Null-arm answer-key hygiene:** `diagnose-transfer` / `diagnose-prompt` use `seedPatch: _agent/diagnose-null-arm-hygiene.patch`. That file is **gitignored** (`_agent/`) and generated on each diagnose parity run (`scripts/regenerate-diagnose-null-arm-hygiene.mjs`), so live worktrees (detached at HEAD) never contain the answer-bearing patch. The prior confound was a tracked delete-patch under `diagnose-outcomes/fixtures/seeds/` that agents foraged for the gate text. Scenario display names stay opaque (`session hunch A/B`); keep `compareId` stable. Outcomes arm keeps skill/suite files (skill-on).
