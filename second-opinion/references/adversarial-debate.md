...
Staged debate recipe for [`second-opinion`](../SKILL.md). Kernel SSOT → via subagents (adversarial) § Staged debate. Spawn mechanics → via subagents non-negotiables.
...
Under an Auto parent, share `inherit-auto` (omit tool `model`); diversify via stances. Distinct explicit models only under named parent (same tier), user cross-model request, or recorded overrides — via subagents (adversarial) § Model routing overlay.

### Wave 2 (sequential, related context)

| Slice    | Subagent         | Stance   | Mandate                                                                                                                 |
| -------- | ---------------- | -------- | ----------------------------------------------------------------------------------------------------------------------- |
| Steelman | `generalPurpose` | `defend` | Rebut, narrow, or concede each **anchored** attacker claim with evidence; ignore unanchored kills unless tagged `drift` |
...
1. Merge per via subagents synthesis gate and via subagents (adversarial) § Coordinator synthesis — **preserve conflicts**.
