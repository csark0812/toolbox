...
### A. Parent Auto — 3-member parallel research
...
- docs-researcher · model=inherit-auto · stance=n/a: topic A
- docs-researcher · model=inherit-auto · stance=n/a: topic B
- explore · model=inherit-auto · stance=n/a: repo map for cited APIs
```

Expected: omit `model` on all Tasks; no `*-fast`; no API slugs.

### B. Named parent, `auto` absent from enum — cost-controlled `N ≥ 2`
...
### C. Sequential Premium escalation (after conflicts)
...
Expected: single Task with explicit Sonnet medium; not high-fast; not parallel with other Premium members.
